    ...
    const std::string& target)
{
    ...
    if (FAILED(hr))
    {
        ...
        std::string filenameUtf8(utf8Length > 0 ? utf8Length : 1, '\0');
        if (utf8Length > 0)
        {
            WideCharToMultiByte(
                CP_UTF8, 0, filename.c_str(), -1,
                &filenameUtf8[0], utf8Length, nullptr, nullptr);
            filenameUtf8.resize(utf8Length - 1);
        }

        std::string msg = "Failed to compile shader: " + filenameUtf8;
